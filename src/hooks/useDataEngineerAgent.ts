...
export interface DEDecisionResult {
  execution_id: string;
  agent_name: string;
  stage: string;
  execution_mode: string;
  de_decision: Record<string, unknown>;
  audit_metadata: Record<string, unknown>;
}

export function useDataEngineerAgent(projectId: string, organizationId: string) {
  const [loading, setLoading] = useState(false);
  const [lastResult, setLastResult] = useState<DEDecisionResult | null>(null);
  ...
  const runCheck = useCallback(async (
    ...
    try {
      ...
      setLastResult(data as DEDecisionResult);
      return data as DEDecisionResult;
    } catch (err: any) {
      toast({
        title: "Erro no Data Engineer Agent",
        description: err.message || "Falha na análise de engenharia de dados",
        variant: "destructive",
      });
      return null;
    } finally {
  ...
  const loadHistory = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq("agent_name", "data_engineer_agent")
