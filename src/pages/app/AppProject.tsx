...
const AppProject = () => {
  ...
  const handleAskLys = () => {
    navigate(`/app/lys?project_id=${projectId}`);
  };
      ...
      <AppShell showBackButton title="Erro" hideBottomNav>
        <div className="flex items-center justify-center p-8">
          <Card className="p-6 text-center max-w-sm">
            ...
            <Button onClick={() => navigate('/app/projetos')}>Voltar</Button>
          </Card>
        </div>
      </AppShell>
    );
  ...
  return (
    <AppShell showBackButton title={project.name} hideBottomNav>
      <div className="p-4 pb-24 space-y-6 max-w-lg mx-auto">
        ...
        {/* Business Impact */}
        <section>
          <h2 className="text-lg font-semibold mb-3">Impacto de Negócio</h2>
            ...
            <CardContent className="p-0">
              {actions.length === 0 ? (
                <div className="p-6 text-center text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Essas campanhas foram registradas na plataforma web e estão sendo monitoradas automaticamente.</p>
                </div>
              ) : (
                      ...
                      <div key={action.id} className="p-4">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <span className="font-medium">
                              {format(new Date(action.start_date), 'dd/MM', { locale: ptBR })}
                              {action.end_date && ` - ${format(new Date(action.end_date), 'dd/MM', { locale: ptBR })}`}
                            </span>
                          </div>
                        </div>
          ...
          <p className="text-xs text-muted-foreground text-center mt-3">
            Essas campanhas foram registradas na plataforma web e estão sendo monitoradas automaticamente.
          </p>
        </section>
      </div>
      ...
      {/* FAB - Ask Lys */}
      <button
        onClick={handleAskLys}
        className="fixed bottom-6 right-6 flex items-center gap-2 px-5 py-3 bg-primary text-primary-foreground rounded-full shadow-lg hover:shadow-xl transition-all active:scale-95"
        style={{ zIndex: 60 }}
      >
        <MessageCircle className="w-5 h-5" />
        <span className="font-medium">Perguntar à Lys</span>
      </button>
    </AppShell>
  );
};
