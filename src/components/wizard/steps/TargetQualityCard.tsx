...
import {
  ...
  TrendingUp, BarChart3, Eye, ShieldAlert, Activity
} from "lucide-react";
...
const scoreBg = (score: number) =>
  score >= 80 ? "bg-accent/10 border-accent/20 text-accent" :
        ...
        <div className="flex items-center gap-2">
          ...
          {report && (
            <Badge className={`text-xs ${scoreBg(report.quality_score)}`}>
              {report.quality_score}/100 — {report.quality_label}
            </Badge>
          )}
        </div>
      ...
      {report && !loading && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {[
              { key: "balance", label: "Equilíbrio", score: report.balance_score },
              { key: "coverage", label: "Cobertura", score: report.coverage_score },
              { key: "stability", label: "Estabilidade", score: report.stability_score },
              { key: "leakage", label: "Anti-Leak", score: report.leakage_score },
            ].map((dim) => (
              <div key={dim.key} className={`flex flex-col items-center p-2 rounded-md border ${scoreBg(dim.score)}`}>
          ...
          {/* Top risks (gates with WARN/BLOCK) */}
