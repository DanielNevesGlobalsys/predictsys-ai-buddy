  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label="ROC-AUC" value={metrics.AUC} baseline={baselineMetrics.AUC} format="pct" />
        <MetricCard label="PR-AUC" value={metrics.pr_auc} baseline={undefined} format="pct" />
        <MetricCard label="F1 Score" value={metrics.F1} baseline={baselineMetrics.F1} format="pct" />
              ...
              <table className="w-full text-[10px]">
                ...
                <tbody>
                  {extended.threshold_curve
                    .filter((_, i) => i % 2 === 0) // Show every other point for readability
                    .map((p, i) => (
...
function MetricCard({ label, value, baseline, format, highlight }: {
  label: string;
  value?: number | null;
  baseline?: number;
  format: "pct" | "num";
  ...
  const displayValue = format === "pct" ? `${(value * 100).toFixed(1)}%` : value.toFixed(4);
  const improvement = baseline != null ? value - baseline : null;
  const isGood = improvement != null ? improvement > 0 : value > 0.6;

  return (
    <div className={`p-3 rounded-lg border text-center space-y-1 ${highlight ? "bg-primary/5 border-primary/20" : "bg-background/60 border-border/50"}`}>
      ...
      {improvement != null && (
        <p className={`text-[10px] ${improvement > 0 ? "text-accent" : "text-destructive"}`}>
          {improvement > 0 ? "+" : ""}{(improvement * 100).toFixed(1)}% vs baseline
        </p>
      )}
    </div>
  );
