-- Fix integer overflow in hash computation
...
begin
  ...
  -- Use md5 for hash instead of manual bit-shifting (avoids integer overflow)
  v_target_hash := 'th_' || left(md5(p_project_id::text || '|' || coalesce(p_target_column,'') || '|' || v_new_version::text), 12);

  insert into public.project_model_selection as pms (
    project_id, organization_id, target_column, problem_type,
    selected_features, excluded_features, selection_version,
    target_hash, updated_at, updated_by
  )
  values (
    p_project_id, p_organization_id, p_target_column, nullif(p_problem_type,''),
    to_jsonb(coalesce(p_selected_features, array[]::text[])),
    to_jsonb(coalesce(p_excluded_features, array[]::text[])),
    v_new_version, v_target_hash, now(), p_user_id
  )
